  ...
  parameters: objectSchema(
    {
      ...
      scope: { type: 'string', enum: ['global', 'person', 'project', 'communication', 'calendar', 'email', 'operational'], description: 'Where the memory applies. Use global unless the user clearly limited it.' },
      scopeRef: { type: 'string', description: 'Normalised person, project or matter association for a specific scope.' },
      expiresAt: { type: 'string', description: 'ISO timestamp only when the user made the preference temporary.' },
    },
    ['type', 'title', 'content'],
  ),
